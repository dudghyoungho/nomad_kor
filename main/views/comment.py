...
from main.models.post import Post  # models/post.py에서 Post 모델 임포트
from main.models.comment import Comment  # models/comment.py에서 Comment 모델 임포트
...
class CommentDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    serializer_class = CommentSerializer
    permission_classes = [IsAuthenticatedOrReadOnly]
    queryset = Comment.objects.all()  # queryset을 추가하거나 None으로 설정
    ...
    @swagger_auto_schema(
        ...
        responses={
            200: openapi.Response(description="조회 또는 수정 성공", schema=openapi.Schema(type=openapi.TYPE_OBJECT)),
    ...
    def get_object(self):
        ...
        # position_id, ftf_id, anonymous_id 중 하나만 있을 경우에만 처리
        if sum([bool(position_id), bool(ftf_id), bool(anonymous_id)]) != 1:
            raise ValidationError({
                "error": "Invalid board type or post in URL. Only one of position_id, ftf_id, or anonymous_id should be provided."
            })
        ...
        try:
            ...
            raise ValidationError({"error": f"Comment not found with ID {comment_id}."})
